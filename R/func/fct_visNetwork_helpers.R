...
add_node <- function(nodedf, id, label = "", size = 20, start = FALSE,
...
add_edge <- function(edgedf, id, label, to, from, type, parameter,
                     ...
                     selfReference.size = 20, hidden = FALSE, color = "black") {
  new_row <- list(
...
add_null_edge <- function(edgedf, nodeid) {
  add_edge(
    ...
    min = NA, max = NA,
    selfReference.angle = pi, selfReference.size = 20, hidden = FALSE,
    color = "black"
  )
}
...
graphdf_to_visNetwork <- function(graphdf, edgeSep = "_", showNull = TRUE,
  ...
  if ("state1_id" %notin% colnames(graphdf)) {
    nodes <- data.table(
      id = node_names, label = node_names, size = 20,
    ...
    edges <- data.table(
      ...
      selfReference.angle = selfReference.angle, selfReference.size = 20, hidden = hidden,
      color = "black"
    )
  } else {
    nodes <- data.table(
      id = unique(c(graphdf$state1_id, graphdf$state2_id)), label = node_names, size = 20,
      start = startbool, end = endbool, shape = shape,
      color.background = "lightblue", color.border = "lightblue",
      shadow = FALSE
    )
...
modify_visNetwork <- function(event, graphdata_visNetwork, addNull = FALSE) {
  ...
  if (event$cmd == "addEdge") {
    graphdata_visNetwork_return$edges <- add_edge(
      ...
      min = "None", max = "None",
      selfReference.angle = 2 * pi, selfReference.size = 20, hidden = FALSE,
      color = "black"
    )
  }
